...
int	check_syntax_errors(char **separate, t_token *ar_token)
{
	...
	while (separate[i])
	{
		if (separate[i][1] && !ar_token[i].quote[0] && ((separate[i][0] == '>' \
			&& separate[i][1] != '>') \
			|| (separate[i][0] == '<' && separate[i][1] != '<')))
			return (print_syntax_error(separate[i] + 1));
		if (!ar_token[i].quote[0] && !ar_token[i - 1].quote[0] && \
			(separate[i][0] == '>' || separate[i][0] == '<' \
			|| separate[i][0] == '|') && (separate[i - 1][0] == '>' \
			|| separate[i - 1][0] == '<'))
			return (print_syntax_error(separate[i]));
		i++;
	}
	if (i > 0 && !ar_token[i - 1].quote[0] && (separate[i - 1][0] == '>' \
		|| separate[i - 1][0] == '<' \
		|| separate[i - 1][0] == '|'))
		return (print_syntax_error(separate[i - 1]));
	return (0);
